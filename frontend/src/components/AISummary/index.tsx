  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-red-50 border border-red-200 rounded-lg">
      <div className="flex items-center mb-2">
        ...
        <h3 className="text-red-700 font-medium">要約の生成エラー</h3>
      </div>
      <p className="text-red-600 text-sm">{error}</p>
      <p className="text-red-500 text-xs mt-2">モデル設定とAPIキーを確認するか、再試行してください。</p>
    </div>
  );
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-blue-700 font-medium">
            {status === 'processing' ? '文字起こしを処理中' : '要約を生成中'}
          </h3>
  ...
  if (!hasContent && status === 'completed') {
    return (
      <div className="w-full p-4 bg-gray-50 border border-gray-200 rounded-lg text-center">
        <p className="text-gray-600">要約コンテンツがありません。</p>
        <p className="text-gray-500 text-sm mt-1">新しい要約を生成してみてください。</p>
      </div>
    );
  }
      ...
      {contextMenu.visible && selectedBlocks.length > 0 && (
        ...
        >
          ...
          >
            ...
            <span>{selectedBlocks.length > 1 ? `${selectedBlocks.length}個のブロック` : 'ブロック'}をコピー</span>
          </button>
          <button
          ...
          >
            <span>🗑️</span>
            <span>{selectedBlocks.length > 1 ? `${selectedBlocks.length}個のブロック` : 'ブロック'}を削除</span>
          </button>
        </div>
      )}
