  ...
  const handleDelete = async (itemId: string) => {
    ...
    try {
      ...
      toast.success("会議を削除しました", {
        description: "関連するすべてのデータが削除されました"
      });
          ...
          {item.type === 'folder' ? (
            <>
              ...
              {searchQuery && item.id === 'meetings' && isSearching && (
                <span className="ml-2 text-xs text-blue-500 animate-pulse">検索中...</span>
              )}
            </>
          ) : (
            <div className="flex flex-col w-full">
              ...
              {hasTranscriptMatch && (
                <div className="mt-1 ml-8 text-xs text-gray-500 bg-yellow-50 p-1.5 rounded border border-yellow-100 line-clamp-2">
                  <span className="font-medium text-yellow-600">一致:</span> {matchingResult.matchContext}
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div className="fixed top-0 left-0 h-screen z-40">
            ...
            {!isCollapsed && (
              <div className="p-3">
                ...
                <div className="relative mb-1">
                  <InputGroup >
                    <InputGroupInput placeholder='会議内容を検索...' value={searchQuery}
      ...
      <ConfirmationModal
        ...
        text="この会議を削除してもよろしいですか？この操作は元に戻せません。"
      ...
      <Dialog open={editModalState.isOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px]">
          <VisuallyHidden>
            <DialogTitle>会議タイトルの編集</DialogTitle>
          </VisuallyHidden>
          <div className="py-4">
            <h3 className="text-lg font-semibold mb-4">会議タイトルの編集</h3>
