    ...
    const setupListeners = async () => {
      ...
      unlistenComplete = await listen<{ modelName: string }>(
        ...
        (event) => {
          ...
          toast.success(`${getModelIcon(model?.accuracy || 'Good')} ${displayName} 準備完了！`, {
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {advancedModels.length > 0 && (
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="advanced-models">
            <AccordionTrigger>
              <span className='text-lg'>高度なモデル</span>
            </AccordionTrigger>
            <AccordionContent>
      ...
      {selectedModel && (
        ...
        >
          {getDisplayName(selectedModel)} で文字起こし中
        </motion.div>
      )}
    </div>
  );
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-600 ml-9 mt-1.5">
              ...
              <span className="flex items-center space-x-1">
                <span>🎯</span>
                <span>{model.accuracy} 精度</span>
              </span>
              <span className="flex items-center space-x-1">
                <span>⚡</span>
                <span>{model.speed} 処理速度</span>
              </span>
            </div>
          </div>
